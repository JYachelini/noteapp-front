...
const ShowNote: React.FC = () => {
	...
	const { isModalEditVisible, noteToEdit, handleModalEdit, handleModalDelete, handleModalArchive, isModalCreateVisible } = useContext(ModalContext)
	...
	if (!isModalEditVisible && !isModalCreateVisible) return null
	else
		return (
				...
				<div className='create-note rounded-md h-full pt-10'>
					<form className='flex flex-col gap-3 h-full'>
						...
						<div className='flex gap-3 px-6 h-[10%]'>
							...
							<div className='rounded-md flex gap-1 p-3 flex-wrap w-full bg-slate-100 overflow-y-auto'>
								{categories
									? categories.map((category, index) => (
											<div key={index} className='categories bg-slate-200 cursor-pointer hover:bg-black hover:text-slate-200 transition-colors h-min ' onClick={() => remove(index)}>
												<span>{category}</span>
